...
void rsa_encrypt_decrypt(const string& filename) {
    ...
    vector<unsigned char> file_ciphertext(plaintext.size() + 16);
    int len;
    ...
    vector<unsigned char> encrypted_key(encrypted_key_len);
    ...
    cout << "Зашифрованный файл сохранён: " << filename << ".rsa.enc" << endl;
    ...
    vector<unsigned char> read_encrypted_key(result.begin() + offset, result.begin() + offset + read_key_len);
    offset += read_key_len;
    
    unsigned char read_nonce[12];
    copy(result.begin() + offset, result.begin() + offset + 12, read_nonce);
    offset += 12;
    
    vector<unsigned char> read_file_ciphertext(result.begin() + offset, result.end());
    ...
    vector<unsigned char> decrypted_key(decrypted_key_len);
    ...
    copy(read_file_ciphertext.end() - 16, read_file_ciphertext.end(), read_tag2);
    vector<unsigned char> actual_file_ciphertext(read_file_ciphertext.begin(), read_file_ciphertext.end() - 16);
    
    EVP_DecryptInit_ex(cipher_ctx, cipher, nullptr, decrypted_key.data(), read_nonce);
    
    vector<unsigned char> decrypted(actual_file_ciphertext.size());
    ...
    cout << "Дешифрованный файл сохранён: " << filename << ".rsa.decrypted" << endl;
